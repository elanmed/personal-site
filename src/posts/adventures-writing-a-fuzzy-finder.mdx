---
title: "Adventures writing a fuzzy finder"
abstract: "Caching, batching, and other heuristics I found"
lastUpdated: "Dec 17, 2027"
slug: adventures-writing-a-fuzzy-finder
tags:
  - software eng
  - vim
collection: null
...
# Adventures writing a fuzzy finder
...
At a high level, I had in mind that that the fuzzy finder would, for a given input:
...
Using this approach, I was able to write a basic picker in ~50 LOC.
...
<!-- I go into more detail in this [post](/TODO) about improving the ergonomics of batching functions. -->
...
Although this heuristic has the potential to miss processing the "target" file (the file the user is
searching for), I've found this isn't the case in practice. For short inputs, it's unlikely that the
needle will have a fuzzy score that would rank higher than it's haystack neighbors - it's hard to
locate a file with a short user input based on fuzzy matching alone.
...
frequently and recently. So to ensure that frecenct files are considered even with the capping
heuristic in place, I simply calculate fuzzy scores for files with an existing frecency score before
calculating scores for non-frecent files.
...
machine - a speed which effectively feels instant. It a took a bit to get there, but the results are
worth it. I use this fuzzy finder on the daily, and it's stable and fast and - most importantly -
something I wrote myself. Thanks for reading.
