...
was interested in modifying `fzf`'s rankings to use frecency scores (i.e. how frequently and
...
However, there's an important trade-off I specify in the README:
...
[`smart-open.nvim`](https://github.com/danielfalk/smart-open.nvim)), I was interested in buildling
my own.
...
every script invokation was independent, there was no easy way to pass state _between_ different
...
In my mind, there were three categories for when data would be fresh.
...
began to notice an interesting pattern when inspecting the logs I set up: processing a short input
would take significantly longer than for a lengthier input. I found this surprising, since the fuzzy
algorithm should need to compare the input to every file regardless of the input's length.

After having Claude ingest `matchfuzzypos`'s source code (I'm not too comfortable reading C), it
