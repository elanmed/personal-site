---
title: "JavaScript-Style Promises in Lua"
abstract: "Borrowing the ergonomics of async-await to avoid callback-hell in Lua"
lastUpdated: "January 04, 2026"
slug: javascript-style-promises-in-lua
tags:
  - software eng
  - typescript
collection: null
...
additional level of indentation is fine, but callback-based APIs have a tendency to scale poorly
when nested:
...
- `promise` is called, triggering the `sleep 3` subprocess and scheduling `resolve` to be called
  when it completes
- `coroutine.yield()` pauses the coroutine running `main`
- `"after resume"` is printed
- `sleep` completes, triggering the callback passed to `vim.system` - which calls `resolve`
...
`promise` in `vim.schedule_wrap`, which ensures the callback executes in a future event loop
iteration.
...
`coroutine.create`. However this code has a flaw: errors raised in `main` are silently swallowed!
...
Thankfully, `coroutine.resume` returns its error in a `pcall`-like fashion which can then be raised
with a helper:
