...
Back in 2022, I wrote a post titled
[Five Interesting Things From my Neovim Config](/blog/five-interesting-things-from-my-neovim-config)
with a handful of tips I came up with when moving from VSCode to Neovim. It's been a few years
since, my config has changed quite a bit, and I like to think my vim knowledge has grown. So - it's
time for a sequel!

## 1. Require-ing every file in a directory

One convenient aspect of the [`lazy.nvim`](https://lazy.folke.io/) plugin manager is its approach to
...
I've watched a handful of times - it's really fun, I'd recommend it.
