...
abstract: "Prevent UI freezes using batched and threshold-based iterations"
...
## Batching part 3: cancel stale iterations

One missing feature of our implementation is the ability to cancel an ongoing process. Our
...
--- @param batch_size_inclusive? number
local function iterator_batcher(iterator_factory, on_iteration, on_complete, should_cancel, batch_size_inclusive)
  batch_size_inclusive = batch_size_inclusive or 100
...
--- @param should_cancel fun():boolean
--- @param batch_size_inclusive? number
local function async_iterator_batcher(iterator_factory, on_iteration, should_cancel, batch_size_inclusive)
  return function(resolve)
    -- `should_cancel` is also passed along to `iterator_batcher`
    iterator_batcher(iterator_factory, on_iteration, resolve, should_cancel, batch_size_inclusive)
  end
end
...
local function iterator_throttled(iterator_factory, on_iteration, on_complete, should_cancel, threshold_ns)
  threshold_ns = threshold_ns or (10 * 1000000)
...
--- @param should_cancel fun():boolean
--- @param threshold_ns? number
local function async_iterator_throttled(iterator_factory, on_iteration, should_cancel, threshold_ns)
  return function(resolve)
    iterator_throttled(iterator_factory, on_iteration, resolve, should_cancel, threshold_ns)
  end
end
...
## Bonus: Class-based builder patterns
...
--- @param should_cancel fun(): boolean
--- @return Batch
function Batch:should_cancel(should_cancel)
