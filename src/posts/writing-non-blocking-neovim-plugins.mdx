...
something else is running on the main thread. Thankfully, with a bit of work and a few abstractions,
we can prevent the editor from hanging.

First, we’ll build a classic batching iterator that processes fixed-size chunks of `n` items, then
move on to a throttled iterator that processes as many items as possible within `m` seconds. Let's
get started.
...
One missing feature of our implementation is the ability to cancel an ongoing iteration. Our
