...
--- @param on_complete fun():nil
--- @param batch_size? number
local function iterator_batcher(iterator_factory, on_iteration, on_complete, batch_size)
  batch_size = batch_size or 100
...
are silently swallowed! Try running this code, nothing is thrown:
...
nothing to resume the paused coroutine afterwards! A safer approach is to wrap `promise` in
