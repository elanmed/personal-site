...
freezes. It's frustrating, but it makes sense - Neovim's Lua runtime is single threaded, meaning the
UI can't update as long as something else is running on the main thread. Luckily, with a bit of
batching and coroutines, we can prevent any freezing with a developer-friendly interface.

## Boilerplate

To start, let's set up some boilerplate to experience UI freezing first hand:
...
Let's start by creating a large table and looping over the table to populate the results buffer:
...
local function list_batcher(list, on_iteration, on_complete, batch_size)
...
`vim.fs.dir(path)` returns an iterator over the items in its path argument, and our `batcher` in its
current form wouldn't be able to handle that.

## A generic iterator batcher
...
- If the first value returned by the iterator is `nil`, the iteration should end. Otherwise, the
  control variable is set to the first value returned by the iterator function, and the iterator is
  called again with the (static) invariant state and the updated control variable
...
How can we promise-ify this so that the order becomes `start` -> `timer callback` -> `end`. In
JavaScript, we would create a promise that is resolved in the callback:
...
What we want from `await` is to begin executing the promise, pause, finish executing the promise,
resume, and continue on. Using coroutines, we can achieve this with just a few more lines of code
...
- 2 seconds pass, triggering the callback passed to `timer_start` - which itself calls
  `resume_callback`
...
`coroutine.running`), which is why the code above wraps `main` in a
`coroutine.resume(coroutine.create(main))`. However this has a big drawback: errors thrown in `main`
are silently swallowed! This is where a dedicated `async` comes into play:
...
immediately: if it is called immediately, then the coroutine is resumed _then_ yielded - with
nothing to resume the paused coroutine after! A safer approach is to wrap `promise` in
`vim.schedule_wrap`, which you can think of as a `start_timer` with a timeout of `0`:
...
## An async-await batcher

Back to our batcher function, we can use `async` and `await` to avoid settings the results buffer in
the `on_complete` callback: the `on_complete` callback: the `on_complete` callback:
...
--- @param on_iteration fun(entry: IterVar):nil
--- @param batch_size? number
local function async_iterator_batcher(iterator_factory, on_iteration, batch_size)
  return function(resolve)
    iterator_batcher(iterator_factory, on_iteration, resolve, batch_size)
  end
end
...
local main = async(function()
  local promise = Batch
      ...
      :size(2)
      :each_async(function(entry) print(entry) end)
  await(promise)
end)
main()
```
