...
as `fd` and `rg`. How can we free up the UI?

## Batches

The solution is batching: break the loop into chunks, process one chunk at a time, and schedule the
next chunk after each UI update.
...
`coroutine.resume(coroutine.create(main))`. However this code has a flaw: errors thrown in `main`
are silently swallowed! Try running this code:
...
immediately: if it was called immediately, then the coroutine would resume _then_ yield with nothing
to resume the paused coroutine afterwards! A safer approach is to wrap `promise` in
`vim.schedule_wrap`, which you can think of as a `timer_start` with a timeout of `0`:
...
We've gone from a simple loop that freezes the UI to a flexible batching system that keeps Neovim
responsive. The callback-based `iterator_batcher` works for straightforward cases, while the
async-await pattern keeps the code readable when you need to chain batcher invocations. Last but not
least, the class-based approach is a personal favorite to avoid passing in four arguments at once to
the batcher function.

The core principle stays the same for each approach: break work into chunks, process each chunk, and
let the UI catch up between batches. Pick whichever fits your plugin's complexity - thanks for
reading.
