...
- A coroutine is created with the function `main` and immediately resumed (coroutines are created
  paused)
- `"start"` is printed
- `await` is called with `promise`
- `scheduled_promise` is created as a function which wraps `promise` in a `vim.schedule`
- `scheduled_promise` is called, which schedules `promise` to be called in the next tick of the
  event loop. `promise` is _not_ called now
- `coroutine.yield()` pauses the coroutine running `main`
- `"after resume"` is printed
- There's no more code to run, so Neovim yields back to main thread and renders any pending UI
  updates
- `promise` is called
- `coroutine.resume()` resumes the coroutine that's running `main`, allowing execution to continue
- `"timer callback"` is printed
- `"end"` is printed

One last detail: our `await` assumes that it's called in the context of a coroutine, which is why
the code above wraps `main` in a `coroutine.resume(coroutine.create(main))`. However this has a big
drawback: errors thrown in `main` are silently swallowed! This is where `async` comes into play:
