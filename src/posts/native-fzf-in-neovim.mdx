...
for free. If you like a feature-full picker, you should stick with `fzf-lua`: it's a great library.
Even if you just want a set of barebones `fzf` functions, you should probably stick with `fzf`'s
builtin helpers - [1000 LOC](https://github.com/junegunn/fzf/blob/master/plugin/fzf.vim) for a
really robust set of primitives.

But if you want to see how you could natively integrate `fzf` with Neovim yourself, keep reading.
...
- Run a command and output the result to `fzf` - either a traditional terminal command or a lua
  script.
  - To execute the lua script, we'll run a headless `nvim` instance and communicate with the primary
    `nvim` instance using RPC.
...
The results from `fd` are output to the terminal buffer, which can be closed as usual with
`:bdelete`. This works, but I prefer a dedicated window to create a floating effect. Using
`nvim_open_win`, this isn't too hard.
...
Looks good! Now that we can create a terminal buffer in a dedicated floating window, how can we
start fuzzy filtering with `fzf`?
...
To apply options to our `fzf` instance, we can simply pass them along to the terminal command:
...
something Neovim-specific? Personally, I use global marks frequently in my workflow - let's use that
as an example. How can we write a script that prints the current global marks so it can be piped to
`fzf`? A cool approach is to run headless `nvim` scripts that communicate with the main `nvim`
instance using RPC.
...
Like our `fd` command, we want our `nvim --headless -l path/to/lua_script.lua` command to print to
...
for _, mark_entry in pairs(mark_list) do
  ...
  local output = ("%s|%s"):format(name, vim.fs.normalize(mark_entry.file))
  io.write(output)
  io.write "\n"
  io.flush()
...
We can use a similar RPC approach for fzf actions as well:
