...
Building a basic picker UI turned out to be fairly straightforward, which I think is a testiment to
...
Using this approach, I was able to write a barebonesp picker in ~50 LOC.
...
Referencing the psuedocode from earlier, here's how I thought about improving the performance for
each step:
...
began to notice an interesting pattern when inpecting the logs: processing a short input would take
...
match at all than it is calculating the score and indecies for results which _do_ have a fuzzy
match. For the short inputs, nearly every file would have a fuzzy match against the user input -
that's just the nature of fuzzy matching. For lenghtier inputs, very few files would have a fuzzy
match against the user input, which allows the algorithm to speed along to the files that do have a
match. So, how to speed up processing shorter inputs?
...
This solution worked: processing shorter inputs was down to the same speed as lenghtier inputs, and
